...
router.get("/:id", async function (req, res, next) {
  try {
    ...
    if (result.rows.length === 0) {
      throw new ExpressError(`No invoice of id: ${id}`,404);
    }
...
router.put("/:id", async function (req, res, next) {
  try {
    ...
    const invoiceResult = await db.query(`SELECT paid FROM invoices WHERE id = $1`, [id]);

    if (invoiceResult.rows.length === 0) {
      throw new ExpressError(`No invoice of id: ${id}`, 404);
    }

    const currentPaidDate = invoiceResult.rows[0].paid_date;
